...
class DefaultBuilderGeneratorTest : UnitSim() {
    ...
    @Test
    fun `generate with DslDescription but no value arg does not add kdoc`() = test {
        given {
            val outputStream = ByteArrayOutputStream()
            val codeGen: CodeGenerator = mockk()
            io.mockk.every {
            ...
            val descAnn: KSAnnotation = mockk()
            io.mockk.every { descAnn.shortName } returns mockKSName("DslDescription")
            io.mockk.every { descAnn.arguments } returns emptyList()
    ...
    @Test
    fun `generate with injected method produces method in output`() = test {
        given {
            val outputStream = ByteArrayOutputStream()
            val codeGen: CodeGenerator = mockk()
            io.mockk.every {
            ...
            // Build a domain with the function in declarations
            val domain: KSClassDeclaration = mockk()
            io.mockk.every { domain.toClassName() } returns ClassName("org.test", "StarShip")
            io.mockk.every { domain.packageName } returns mockKSName("org.test")
            io.mockk.every { domain.simpleName } returns mockKSName("StarShip")
            ...
            whenever {
                generator.generate(codeGen, domain, config, emptyMap(), false)
                val output = outputStream.toString()
                output.contains("fun designation()") && output.contains("String")
            }
        }
    }

    @Test
    fun `generate with block body injected method produces method in output`() = test {
        given {
            val outputStream = ByteArrayOutputStream()
            val codeGen: CodeGenerator = mockk()
            io.mockk.every {
            ...
            val tempFile = File.createTempFile("TestDomain", ".kt")
            tempFile.writeText("fun greeting(): String {\n    return \"Hello \$name\"\n}")
            tempFile.deleteOnExit()
            ...
            val fnDecl: KSFunctionDeclaration = mockk()
            io.mockk.every { fnDecl.simpleName } returns mockKSName("greeting")
            io.mockk.every { fnDecl.annotations } returns sequenceOf(fnAnn)
            io.mockk.every { fnDecl.returnType } returns fnReturnTypeRef
            io.mockk.every { fnDecl.parameters } returns emptyList()
            ...
            whenever {
                generator.generate(codeGen, domain, config, emptyMap(), false)
                val output = outputStream.toString()
                output.contains("fun greeting()") && output.contains("String")
            }
        }
    }

    @Test
    fun `generate with no injected methods works normally`() = test {
        given {
            val outputStream = ByteArrayOutputStream()
            val codeGen: CodeGenerator = mockk()
            io.mockk.every {
    ...
    @Test
    fun `generate skips injected method with null returnType`() = test {
        given {
            val outputStream = ByteArrayOutputStream()
            val codeGen: CodeGenerator = mockk()
            io.mockk.every {
            ...
            val fnDecl: KSFunctionDeclaration = mockk()
            io.mockk.every { fnDecl.simpleName } returns mockKSName("broken")
            io.mockk.every { fnDecl.annotations } returns sequenceOf(fnAnn)
            io.mockk.every { fnDecl.returnType } returns null
            io.mockk.every { fnDecl.parameters } returns emptyList()
            ...
            whenever {
                generator.generate(codeGen, domain, config, emptyMap(), false)
                val output = outputStream.toString()
                !output.contains("fun broken()")
            }
        }
    }

    @Test
    fun `generate skips injected method with NonExistLocation`() = test {
        given {
            val outputStream = ByteArrayOutputStream()
            val codeGen: CodeGenerator = mockk()
            io.mockk.every {
            ...
            io.mockk.every { fnDecl.returnType } returns fnReturnTypeRef
            io.mockk.every { fnDecl.parameters } returns emptyList()
            io.mockk.every { fnDecl.location } returns NonExistLocation
    ...
    @Test
    fun `generate skips injected method with nonexistent source file`() = test {
        given {
            val outputStream = ByteArrayOutputStream()
            val codeGen: CodeGenerator = mockk()
            io.mockk.every {
            ...
            val fileLocation: FileLocation = mockk()
            io.mockk.every { fileLocation.filePath } returns "/nonexistent/path/Fake.kt"
            io.mockk.every { fileLocation.lineNumber } returns 1
            io.mockk.every { fnDecl.location } returns fileLocation
    ...
    @Test
    fun `generate skips injected method with line number too large`() = test {
        given {
            val outputStream = ByteArrayOutputStream()
            val codeGen: CodeGenerator = mockk()
            io.mockk.every {
            ...
            val tempFile = File.createTempFile("TestDomain", ".kt")
            tempFile.writeText("fun foo(): String = \"hello\"")
            tempFile.deleteOnExit()
            ...
            val fileLocation: FileLocation = mockk()
            io.mockk.every { fileLocation.filePath } returns tempFile.absolutePath
            io.mockk.every { fileLocation.lineNumber } returns 999
            io.mockk.every { fnDecl.location } returns fileLocation
    ...
    @Test
    fun `generate skips injected method with line number zero`() = test {
        given {
            val outputStream = ByteArrayOutputStream()
            val codeGen: CodeGenerator = mockk()
            io.mockk.every {
            ...
            val fileLocation: FileLocation = mockk()
            io.mockk.every { fileLocation.filePath } returns tempFile.absolutePath
            io.mockk.every { fileLocation.lineNumber } returns 0
            io.mockk.every { fnDecl.location } returns fileLocation
    ...
    @Test
    fun `generate skips injected method parameter with null name`() = test {
        given {
            val outputStream = ByteArrayOutputStream()
            val codeGen: CodeGenerator = mockk()
            io.mockk.every {
            ...
            val tempFile = File.createTempFile("TestDomain", ".kt")
            tempFile.writeText("fun greet(greeting: String): String = \"\$greeting\"")
            tempFile.deleteOnExit()
            ...
            // Mock a parameter with null name
            val paramTypeRef: KSTypeReference = mockk()
            io.mockk.every { paramTypeRef.toTypeName() } returns STRING
            val nullNameParam: KSValueParameter = mockk()
            io.mockk.every { nullNameParam.name } returns null
            io.mockk.every { nullNameParam.type } returns paramTypeRef

            val fnDecl: KSFunctionDeclaration = mockk()
            io.mockk.every { fnDecl.simpleName } returns mockKSName("greet")
            io.mockk.every { fnDecl.annotations } returns sequenceOf(fnAnn)
            io.mockk.every { fnDecl.returnType } returns fnReturnTypeRef
            io.mockk.every { fnDecl.parameters } returns listOf(nullNameParam)
            ...
            whenever {
                generator.generate(codeGen, domain, config, emptyMap(), false)
                val output = outputStream.toString()
                // Method should still be generated, just without the null-named param
                output.contains("fun greet()")
            }
        }
    }
}
